...
public class RedAguaPotable2 {
	GeneralTree<Character> arbol;

	public RedAguaPotable2(GeneralTree<Character> arbol) {
		super();
		this.arbol = arbol;
	}
	...
	public double minimoCaudal(double caudal) {
		if (!arbol.isEmpty()) {
			return minCaudalHelper(caudal, arbol);
		} else {
			return -1;
		}
	}
	
	private double minCaudalHelper(double caudal, GeneralTree<Character> nodo) {
		if (nodo.isLeaf()) {
			return caudal;
		}
		double min=9999;
		for (GeneralTree<Character> child: nodo.getChildren()) {
			min = Math.min(min, minCaudalHelper(caudal / nodo.getChildren().size(), child));
		}
		return min;
	}
	 ...
	 public static void main (String[] args) {
	        ...
	        RedAguaPotable2 red = new RedAguaPotable2(ab);
